...
class MGRANet_teacher(nn.Module):
    def __init__(self, channel=64):
        super(MGRANet_teacher, self).__init__()
        # Backbone
...
if __name__ == '__main__':
    ...
    model = MGRANet_teacher().to(torch.device("cuda:0"))
    new_weights = model.state_dict().items()
    # for k, v in new_weights:
    #     print(k, v.shape)
    print('Newmodel.state_dict().items()', len(model.state_dict().items()))
    path = "/home/wby/Desktop/项目框架/RGBDMirrorSegmentation/toolbox/models/MGRANet/243model.pth"
